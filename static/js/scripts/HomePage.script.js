...
export default class {
  constructor(params) {
    ...
    this.bannerSliderPrevArrow = document.querySelector(
      ".slider__items .items__arrow.prev",
    );
    this.bannerSliderNextArrow = document.querySelector(
    ...
    //Main categories
    this.categoriesList = document.querySelector(
      ".list-categories__items .items",
    );
    this.categoriesListPrevArrow = document.querySelector(
      ".list-categories__arrow.prev",
    );
    this.categoriesListNextArrow = document.querySelector(
      ".list-categories__arrow.next",
    );
    this.mainCategories;
    //Trending keywords
    this.keywordsList = document.querySelector(
      ".trending-keywords__list .list__items",
    );
    this.keywordsListMore = document.querySelector(
      ".trending-keywords__title .more",
    );
    this.trendingKeywords;
    this.keywordsIndex = 0;
    //Flashsale item
    this.flashsaleList = document.querySelector(
      ".flashsale__list .list__items .items",
    );
    this.flashsaleListPrevArrow = document.querySelector(
      ".list__items__arrow.prev",
    );
    this.flashsaleListNextArrow = document.querySelector(
      ".list__items__arrow.next",
    );
    this.flashsaleItems;
    //Products list
    this.listProductCategories = document.querySelector(
      ".homepage__products .products__title",
    );
    this.listProducts = document.querySelector(".products__list .list__items");
    this.viewMoreBtn = document.querySelector(
      ".products__list .list__view-more",
    );
  ...
  nextList(list, prevBtn, nextBtn, infinite = false) {
    ...
    if (!infinite) {
      scrollNum +=
        scrollWidth - eleWidth >= eleWidth ? eleWidth : scrollWidth - eleWidth;
  ...
  renderFlashsaleBanner(data) {
    if (data) {
      ...
      this.bannerSlider.innerHTML = data
        ...
        .join("");
      this.bannerRight.innerHTML += `<img src='${
        data[data.length - 2].banner
      }'></img>`;
      this.bannerRight.innerHTML += `<img src='${
        data[data.length - 1].banner
      }'></img>`;
      this.bannerDots.innerHTML = data
      ...
      const autoSlide = setInterval(() => {
        ...
        this.dotIndex =
          this.dotIndex == childs.length - 1 ? 0 : ++this.dotIndex;
        changeStateDot(childs[this.dotIndex]);
        this.nextList(
          this.bannerSlider,
      ...
      this.bannerSliderPrevArrow.addEventListener("click", e => {
        ...
        changeStateDot(childs[this.dotIndex]);
        this.dotIndex =
          this.dotIndex == 0 ? childs.length - 1 : --this.dotIndex;
        changeStateDot(childs[this.dotIndex]);
      });
          ...
          data[i + this.keywordsIndex * 5].count / 1000
                    ...
                    <img src='${
                      data[i + this.keywordsIndex * 5].images[
                        Math.floor(
                          Math.random() *
                            data[i + this.keywordsIndex * 5].images.length,
                        )
                      ]
                    }' href="/search?${
          data[i + this.keywordsIndex * 5].keyword
  ...
  renderProductCategories(mainCategories, apiKey) {
    ...
    while (true) {
      let categoryObj =
        this.mainCategories[
          Math.floor(Math.random() * this.mainCategories.length)
        ];
      let category = {
