...
export default class {
                        ...
                        <span class="item__price">
                            ${
                                item.price_max == item.price_min
                                    ? item.discount == null
                                        ? '<span class="currency">đ</span>' + format(item.price)
    ...
    renderFilterList(data) {
        const categoryGroup = document.querySelector('.filter-group[group="category"] .list-items');
        const locationGroup = document.querySelector('.filter-group[group="location"] .list-items');
        const shippingGroup = document.querySelector('.filter-group[group="shipping"] .list-items');
        const brandGroup = document.querySelector('.filter-group[group="brand"] .list-items');
        categoryGroup.innerHTML = data.category_filters
            .map?.(
                ...
                <label for="${item.category_id}">
                    ...
                    <i class="far fa-check-square hide check" data-id="${item.category_id}"></i>
                    ${item.display_name} 
    ...
    getFilter(group) {
        ...
        arrayElements.forEach(item => {
            item.addEventListener("click", e => {
                ...
                if (input.checked) {
                    document.querySelectorAll(`i[data-id="${input.id}"]`)[0].classList.add("hide");
                    document
                    ...
                    document
                        ...
                        .classList.remove("hide");
                    document.querySelectorAll(`i[data-id="${input.id}"]`)[1].classList.add("hide");
                }
                switch (group) {
                    case "location":
                        if (!input.checked)
        ...
        document.querySelectorAll(".sort[data-sort]").forEach(item => {
            item.addEventListener("click", () => {
                ...
                if (!activeElement) {
                    ...
                    document.querySelector(".sort-price").children[0].classList.remove("highlight");
                    document.querySelector(`[data-sort="${this.sortBy}"]`).children[0].remove();
                } else activeElement.classList.remove("active");
                if (item.classList.contains("price")) {
                    document.querySelector(".sort-price").children[0].classList.add("highlight");
